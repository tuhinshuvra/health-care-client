...
export async function createDoctor(_prevState: any, formData: FormData) {
    try {
        ...
        if (zodValidators(payload, createDoctorZodSchema).success === false) {
            return zodValidators(payload, createDoctorZodSchema);
        }

        const validatedPayload = zodValidators(payload, createDoctorZodSchema).data;
...
export async function updateDoctor(id: string, _prevState: any, formData: FormData) {
    try {
        ...
        const validatedPayload = zodValidators(payload, updateDoctorZodSchema).data;
